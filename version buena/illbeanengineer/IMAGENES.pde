void IMAGENES() {
  ...
  if (solsol==true) {
    ...
    if (countersol>=1 && countersol<20) { 
      sol=sol1;
    }///ANIMACION SOL
    else if (countersol>=20 && countersol<40) { 
      sol=sol2;
    } else if (countersol>=40 && countersol<=60) { 
      sol=sol3;
    }
    countersol++;
    if (countersol==60) { 
  ...
  if (floresmenuu) {
    ...
    for(int flores=0; flores<20; flores++){
      if(randomfloresmenu[flores]>=0 && randomfloresmenu[flores]<1){
      floresmenu[flores]=flor1;}
      if(randomfloresmenu[flores]>=1 && randomfloresmenu[flores]<2){
      floresmenu[flores]=flor2;}
      if(randomfloresmenu[flores]>=2 && randomfloresmenu[flores]<3){
      floresmenu[flores]=flor3;}
      if(randomfloresmenu[flores]>=3 && randomfloresmenu[flores]<=4){
      floresmenu[flores]=flor4;}
      
    }
    
    for (int flores=0; flores<20; flores++) {
      yfloresmenu[flores]=yimg+300+(150*u)+yrandomfloresmenu[flores];
      u++;
        if(u==10){
          u=0;
        }
    ...
    for (int flores=0; flores<20; flores++) {
      if (flores<10) {
        xfloresmenu[flores]=150+xrandomfloresmenu[flores];
      }
      if (flores>=10 && flores<20) {
        xfloresmenu[flores]=850+xrandomfloresmenu[flores];
      }
    }
    if(counterwflores>=0 && counterwflores<20){
      wfloresmenu++;
    }
    else if(counterwflores>=20 && counterwflores<=40){
      wfloresmenu--;
    }
    counterwflores++;
    if(counterwflores==40){
      counterwflores=0;
    }
    
   for(int flores=0; flores<20; flores++){
     imageMode(CENTER);
    image(floresmenu[flores],xfloresmenu[flores],yfloresmenu[flores],wfloresmenu,wfloresmenu) ;
...
if (estrellitas==true) {
  ...
  for (int estrellas=1; estrellas<40; estrellas++) {
    image(stars, xstars[estrellas], ystars[estrellas], weighttt, weighttt);
  } //estrellas
